...
Package-scoped copy used by the app for template & project persistence.
"""
...
def export_sections(
    ...
    """Render export/template/project controls and return template dict."""
    tmpl = build_template_dict({
        ...
        "format": {
            ...
            "title_position": st.session_state.get(
    ...
    with col3:
        if container.button("Download Project (.olite)"):
            proj_path = save_project(
                "current.olite", dm, tmpl, name=chart_title or "project"
            )
        ...
        up_proj = container.file_uploader(
        ...
        if up_proj:
            ...
            meta = extra.get('meta', {})
            pname = meta.get('name', tmp_path.stem)
            container.success(
                f"Project '{pname}' loaded with {len(dm_loaded.df)} rows."
            )
            ...
            if container.button("Apply project chart config"):
